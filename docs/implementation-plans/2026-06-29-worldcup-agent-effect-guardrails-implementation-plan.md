...
7. Concise answer style hardening before live re-evaluation
   ...
   - Behavior change: instruct the model to answer conclusion-first with the four-field side-panel pattern by default (`结论`, `关键数据`, `依据`, `状态/风险`), avoid Markdown tables, long section scaffolding, full 9D lists, Top5 score lists, and market-depth dumps unless explicitly requested, convert default Markdown table rows into plain short lines while removing table scaffolding/horizontal rules before streaming, clamp default verbose generated output while preserving raw-output truncation detection, expose missing central-data base URL as client-unavailable instead of a synthetic transport failure, return only stable public methodology via local fallback when central methodology is unconfigured, and keep DockerHost API/worker provider defaults aligned.
