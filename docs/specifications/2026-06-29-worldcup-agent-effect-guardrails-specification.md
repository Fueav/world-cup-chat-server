...
- Functional:
  ...
  - Prompt and run-scoped instructions enforce concise side-panel answer style: conclusion first, four short fields by default (`结论`, `关键数据`, `依据`, `状态/风险`), no default Markdown tables, long section scaffolding, full 9D lists, Top5 score lists, market-depth dumps, or step-by-step report prose unless the user explicitly asks for detail, expansion, a table, all dimensions, or a comparison.
  - Current-match tool results include the same concise answer contract so large central-data payloads do not cause default report-style answers.
