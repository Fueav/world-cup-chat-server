...
2. `POST /api/v1/wc2026/chat?user_uuid=<user_uuid>`
3. 保存返回的 conversation_id 和 agent_run_id
4. 订阅 stream_url，读取 TOKEN 和 RUN_COMPLETED
5. 用户继续追问时，再调用同一个 chat URL，并传 conversation_id
6. 如果页面刷新或客户端丢失状态，调用 versioned conversations 接口找回会话
```
...
不传 `conversation_id`，服务会自动创建新会话。WC2026 chat endpoint 只做异步受理，
...
4. 调用 `/api/v1/wc2026/chat` 时带 `Idempotency-Key`。
...
10. 页面刷新或本地状态丢失时，调用 versioned conversations 列表找回会话。
11. 需要完整历史时，调用 versioned conversation detail。
12. SSE 断线时，用最后一个 SSE `id` 作为 `Last-Event-ID` 重连。
13. 遇到 `STREAM_GAP`，改查 versioned runs 和 conversations。
