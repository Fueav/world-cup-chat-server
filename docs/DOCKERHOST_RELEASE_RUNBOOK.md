...
如果目标环境复用已有 Postgres volume,在切到包含 WC2026 conversation-match
绑定的版本前，先执行仓库内 migration:

```text
app/db/migrations/2026-06-30-wc2026-conversation-match-binding.sql
```

新建 DockerHost 环境会通过 `app/db/init.sql` 自动创建同样的列和索引。
...
```bash
export USER_UUID="dockerhost-release-smoke"
...
cat >/tmp/wc2026_smoke_body.json <<'JSON'
{
  ...
  "wc2026_context": {
    ...
    "current_match": {
      "id": "83",
      "fd_match_id": "83",
      "description": "墨西哥 vs 厄瓜多尔",
      "stage": "group",
      "stage_label": "小组赛",
      "home": {"name": "墨西哥", "short_name": "MEX"},
      "away": {"name": "厄瓜多尔", "short_name": "ECU"},
      "is_unlocked": true
    },
    "entitlements": {"has_all": false, "unlocked_matches": ["83"], "locked_matches": []}
  }
}
JSON
...
curl -fsS \
  "$BASE_URL/api/v1/wc2026/runs/$RUN_ID?user_uuid=$USER_UUID"
```
...
`GET /api/v1/wc2026/chat/ws/<run_id>?user_uuid=... HTTP/1.1` 而不是 WebSocket upgrade,说明请求已到达 API,
...
async def main() -> None:
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.post(
            "http://localhost:8080/api/v1/wc2026/chat",
            params={"user_uuid": "dockerhost-release-smoke"},
            json={
    ...
    async with websockets.connect(
        f"ws://localhost:8080/api/v1/wc2026/chat/ws/{run_id}?user_uuid=dockerhost-release-smoke",
        open_timeout=15,
        close_timeout=5,
    ) as ws:
        for _ in range(40):
...
- 公网 WebSocket 失败但容器内 `ws://localhost:8080/api/v1/wc2026/chat/ws/<run_id>?user_uuid=...` 成功时,
