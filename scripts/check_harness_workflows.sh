...
"$PYTHON_BIN" - "$MANIFEST" "$DOC" "$SOURCE_DOC" "$VIRTUAL_REQUIREMENTS" "$SPECS_ROOT" "$PLANS_ROOT" "$ARTIFACT_DIR" <<'PY'
...
specs_root = Path(sys.argv[5])
plans_root = Path(sys.argv[6])
...
def validate_bindings(
    root: Path, label: str, workflow_ids: set[str]
) -> dict[str, str]:
...
for index, source in enumerate(source_set):
    ...
    if non_empty_string(source_url):
        if not str(source_url).startswith(("https://", "http://")):
            add_error(f"{context}.url must be absolute: {source_url}")
...
for index, workflow in enumerate(classes):
    ...
    if not non_empty_string(workflow_id) or not str(workflow_id).startswith("HARNESS-"):
        add_error(f"{context}.id must be a HARNESS-* string")
        workflow_id = f"<missing-{index}>"
    elif str(workflow_id) in workflow_by_id:
        add_error(f"duplicate workflow id: {workflow_id}")
    else:
        workflow_ids.append(str(workflow_id))
        workflow_by_id[str(workflow_id)] = workflow
        if doc_text and str(workflow_id) not in doc_text:
    ...
    workflow_source_bindings[str(workflow_id)] = workflow_sources
    ...
    workflow_principle_bindings[str(workflow_id)] = workflow_principles
